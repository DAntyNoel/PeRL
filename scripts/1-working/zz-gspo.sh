...
# kill all vllm and sglang processes
# pkill -9 python || true;
# pkill -9 -f "vllm.entrypoints.openai.api_snerver" || true;
# pkill -9 -f "sglang.server.sglang_server" || true;
...
export WANDB_MODE=offline # wandb offline
debug=true
backend=fsdp2 # fsdp, fsdp2, megatron
project_name=gspo
experiment_name=qwen2.5-0.5B-base-grpo-$backend
timestamp=$(date +%Y%m%d_%H%M%S)
default_local_dir=/home/fhshao/PeRL/outputs/${timestamp}_${experiment_name}
default_local_dir=outputs/debug
...
train_files=/home/fhshao/downloads/data/dapo-math-17k.parquet
test_files=/home/fhshao/downloads/data/aime-2024.parquet

actor_model_path=/home/fhshao/downloads/models/Qwen/Qwen2.5-0.5B
...
ROLLOUT_CONFIG="
    ...
    actor_rollout_ref.rollout.mode=colocate \
